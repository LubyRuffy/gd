...
var (
	levelStrings = [...]string{"FNST", "FINE", "DEBUG", "TRACE", "INFO", "WARN", "ERROR", "FATAL"}
)
...
func (log Logger) intLogfTag(tag, clientIp, logId string, lvl Level, format string, args ...interface{}) {
	skip := true
...
func (log Logger) intLogc(lvl Level, closure func() string) {
	...
	for _, filter := range log {
		...
		if !found {
			_, ok := filter.LogWriter.(*FileLogWriter)
			if ok {
				found = true
			}
		}
...
func (log Logger) intLogcTag(tag, clientIp, logId string, lvl Level, closure func() string) {
	skip := true
...
// Send a log message with manual level, source, and message.
func (log Logger) LogWithTag(tag string, lvl Level, source, message string) {
	skip := true
...
func (log Logger) IsEnabledFor(lvl Level) bool {
	e := false
	for _, filter := range log {
		if lvl >= filter.Level {
			e = true
